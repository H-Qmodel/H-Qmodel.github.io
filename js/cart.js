...
$(function(){
	var Cart = {
		...
		init: function(){
			...
			$.getJSON('js/data.json?key='+Math.random(),function(data){
				...
				for(var key in that.cart){
					//利用闭包保留key值
					(function(k){
						var ul = $('<ul id="brand_cart_22" ></ul>');
						
						ul.load('shopping01.html?key='+Math.random(),function(){
							//获取商品id
							...
							//信息填充
							ul.find('.shop_product_size').html( data[gid]['size'][k]);
							ul.find('.cprice>span').html( data[gid]['goods-sale'].toFixed(2));
							ul.find('#J_AmountWidget>input').val( that.cart[k].amount );
							var total = that.cart[k].amount * data[gid]['goods-sale'];
							ul.find('.shop_product_money00').html( total.toFixed(2) );
							//追加到商品区
		...
		increase: function(){
			...
			this.cartCon.on("click",'.increase',function(){
				...
				//获取商品id和库存
				var gid = $(this).parents('#brand_cart_22').data('gid');
		...
		input: function(){
			var that = this;
			//+点击
			this.cartCon.on("input",'#J_AmountWidget>span',function(){
				//input是自己的前一个兄弟
				var amount = $(this).val();
		...
		handleCookie: function(input){
			var goodsItem = input.parents('#brand_cart_22');
			var sizeId = goodsItem.data('sizeid');
			
			//处理总价
			var price = parseFloat(goodsItem.find('#brand_cart_22').html() );
			var totalMoneyBox = goodsItem.find('.shop_product_money00');
			//重新显示单价商品总价
		...
		remove: function(){
			...
			this.cartCon.on('click','.delete',function(){
				if( confirm('确定删除宝贝吗？') ){
					//当前商品从页面消失
					$(this).parents('#brand_cart_22').remove();
					//从cookie中删除
					var sizeId = $(this).parents('#brand_cart_22').data('sizeid');
					//删除  (复习delete)
	...
	/*	goodsSelect: function(){
			...
			this.cartCon.on('change','.td-checkbox input[type="checkbox"]',function(){
				var goodsItem = $(this).parents('#brand_cart_22');
				//获取商品id
				var sizeId = goodsItem.data('sizeid');
				//总价
		
		var total = goodsItem.find('.shop_product_money00').html();
				//如果已经存在，再点击取消
				if(!goodsItem.find('input[type="checkbox"]').prop("checked")){
		...
		handlePay: function(){
			...
			var goodsMoney = $('.td_len1');
			var goPay = $('.shopping_car_bottom_rightMoney');
			//遍历pay对象，获取件数和总价
			var totalNum = 0;
			var totalMoney = 0;
	...
	/*	selectAll: function(){
		...
		delSelected: function(){
			var that = this;
			$('.shop_product_close .deleted').click(function(){
				var allChecked = that.cartCon.find('input[type="checkbox"]:checked');
